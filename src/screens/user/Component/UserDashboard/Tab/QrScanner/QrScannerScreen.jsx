// Real camera-based QR scanner (react-native-vision-camera). Opened from the
// QR icon in the dashboard header, PhonePe-style — scans any QR code (e.g. a
// doctor/clinic appointment QR) and surfaces the decoded value. There is no
// backend endpoint yet to resolve a scanned code into an appointment record,
// so a successful scan is shown to the user rather than auto-navigated.
...
export default function QrScannerScreen({ visible, onClose, onScanned }) {
  ...
  const handleScanned = useCallback((value) => {
    ...
    if (onScanned) {
      ...
      Alert.alert('QR Code Scanned', value, [
        { text: 'Scan Again', onPress: () => { scannedRef.current = false; setIsActive(true); } },
  ...
  // Reset scan-guard and re-arm the camera each time the scanner is reopened.
  ...
  return (
    <Modal visible={visible} animationType="slide" statusBarTranslucent navigationBarTranslucent onRequestClose={onClose} onShow={handleShow}>
      <View style={s.screen}>
        ...
        <SafeAreaView style={s.overlay} edges={['top', 'bottom']}>
          <View style={s.header}>
            ...
            <Pressable onPress={() => setTorchOn((v) => !v)} style={s.iconBtn} hitSlop={10}>
...
const s = StyleSheet.create({
  ...
  hint: { marginTop: 22, fontSize: 13.5, color: '#FFF', opacity: 0.85, textAlign: 'center', paddingHorizontal: 30 },
  permissionBox: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32, gap: 10 },
  ...
  permissionBtn: { marginTop: 10, height: 46, paddingHorizontal: 24, borderRadius: 23, backgroundColor: PATIENT.primary, alignItems: 'center', justifyContent: 'center' },
