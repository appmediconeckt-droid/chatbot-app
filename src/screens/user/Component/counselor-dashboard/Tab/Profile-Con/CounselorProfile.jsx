...
const CounselorProfile = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Validation: Check if verification documents are uploaded
      if (documents.length === 0) {
        Alert.alert('Required', 'Please upload at least 1 verification document before saving profile');
        setLoading(false);
        return;
      }
      ...
      // Update rest of profile without photo
            ...
            {isEditing && (
              <View style={styles.addCertForm}>
                ...
                <TouchableOpacity
                  ...
                  style={[styles.addCertBtn, documents.length === 0 && styles.addCertBtnDisabled]}
                  disabled={documents.length === 0}
                >
                  <Text style={[styles.addCertBtnText, documents.length === 0 && styles.addCertBtnTextDisabled]}>
                    Add Certification
                  </Text>
                </TouchableOpacity>
                {documents.length === 0 && (
                  <Text style={styles.certRequiredNote}>* Upload verification documents first</Text>
                )}
              </View>
            )}
          </View>
