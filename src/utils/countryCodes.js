export const COUNTRY_CODES = [
  ...
  { name: 'British Virgin Islands', iso2: 'VG', code: '+1-284' },
  ...
  { name: 'Democratic Republic of the Congo', iso2: 'CD', code: '+243' },
  ...
  { name: 'Dominican Republic', iso2: 'DO', code: '+1-829' },
  { name: 'Dominican Republic', iso2: 'DO', code: '+1-849' },
  ...
  { name: 'Puerto Rico', iso2: 'PR', code: '+1-939' },
  ...
  { name: 'Saint Barthelemy', iso2: 'BL', code: '+590' },
  { name: 'Saint Helena', iso2: 'SH', code: '+290' },
  ...
  { name: 'US Virgin Islands', iso2: 'VI', code: '+1-340' },
...
const sortedCountryCodes = [...COUNTRY_CODES].sort(
  (a, b) => codeDigits(b.code).length - codeDigits(a.code).length,
);

export const normalizePhoneDigits = (value, maxLength = 15) =>
  String(value || '').replace(/\D/g, '').slice(0, maxLength);

export const normalizeLocalPhoneNumber = (value, countryCode = DEFAULT_COUNTRY_CODE) => {
...
export const splitInternationalPhoneNumber = (value, fallbackCode = DEFAULT_COUNTRY_CODE) => {
  ...
  const fallback = normalizeCountryCode(fallbackCode);
  if (!raw) return { countryCode: fallback, phoneNumber: '' };

  const digits = normalizePhoneDigits(raw);
  if (!raw.startsWith('+')) {
    return {
      countryCode: fallback,
      phoneNumber: normalizeLocalPhoneNumber(digits, fallback),
    };
  }

  const match = sortedCountryCodes.find((country) => digits.startsWith(codeDigits(country.code)));
  const countryCode = match?.code || fallback;
  const nationalNumber = match ? digits.slice(codeDigits(match.code).length) : digits;

  return {
    countryCode,
    phoneNumber: normalizeLocalPhoneNumber(nationalNumber, countryCode),
  };
};

export const buildInternationalPhoneNumber = (countryCode, phoneNumber) => {
  const normalizedCode = normalizeCountryCode(countryCode);
  const phoneDigits = normalizeLocalPhoneNumber(phoneNumber, normalizedCode);
  return `${normalizedCode.replace(/[^\d+]/g, '')}${phoneDigits}`;
};
...
export const findCountryByCode = (countryCode) => {
  const normalized = normalizeCountryCode(countryCode);
  return COUNTRY_CODES.find((country) => country.code === normalized) || COUNTRY_CODES.find((country) => country.code === DEFAULT_COUNTRY_CODE);
};
